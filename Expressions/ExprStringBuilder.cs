...
namespace NP.Concepts.Expressions
{
    ...
    /// <summary>
    ///  this class is almost an exact copy of ExpressionStringBuilder internal sealed class of 
    ///  .NET 7.1 (is that class was not internal and sealed - I could have used it instead)
    /// </summary>
    public class ExprStringBuilder : ExpressionVisitor
    {
        ...
        internal static string ExpressionToString(Expression node)
        {
            ...
            ExprStringBuilder esb = new ExprStringBuilder();
            esb.Visit(node);
            return esb.ToString();
        }
        ...
        protected override Expression VisitLambda<T>(Expression<T> node)
        {
            ...
            {
                // p => body
                Visit(node?.Parameters.FirstOrDefault());
            }
            else
            {
                // (p1, p2, ..., pn) => body
